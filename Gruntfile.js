...
module.exports = (grunt) => {
  ...
  grunt.initConfig({
    ...
    concat: {
      license: {
        ...
        files: {
          'dist/tinymce-geogebra/plugin.js': [
            'src/text/license-header.js',
            scratchPluginMinPath
          ],
          'dist/tinymce-geogebra/plugin.min.js': [
            'src/text/license-header.js',
            scratchPluginMinPath
          ]
        }
    ...
    copy: {
      css: {
        files: [
          { src: [ 'CHANGELOG.txt', 'LICENSE.txt' ], dest: 'dist/tinymce-geogebra', expand: true }
        ]
      }
    },
  ...
  grunt.registerTask('version', 'Creates a version file', () => {
    grunt.file.write('dist/tinymce-geogebra/version.txt', BUILD_VERSION);
  });
